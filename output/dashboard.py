...
/* ── Masquer l'indicateur de chargement natif (topbar) ──────────────── */
[data-testid="stStatusWidget"] { display: none !important; }

/* ── Spinner Streamlit → overlay plein écran ─────────────────────────── */
...
[data-testid="stSpinner"] > div {
    ...
    box-shadow: 0 0 100px rgba(0, 200, 255, 0.10) !important;
}
[data-testid="stSpinner"] svg {
    width: 72px !important; height: 72px !important;
    color: #00c8ff !important;
    filter: drop-shadow(0 0 16px rgba(0, 200, 255, 0.6)) !important;
}
...
# ---------------------------------------------------------------------------
# Blocage anti-spam des boutons (5 s) — JS via composant iframe
# L'overlay visuel est géré par le CSS stSpinner ci-dessus
# ---------------------------------------------------------------------------
import streamlit.components.v1 as _stc
_stc.html("""
<script>
(function() {
    try {
        ...
        if (pdoc.getElementById('fa-btn-guard')) return;
        var g = pdoc.createElement('span');
        g.id = 'fa-btn-guard';
        g.style.display = 'none';
        pdoc.body.appendChild(g);

        var lockUntil = 0;

        function lockButtons() {
            pdoc.querySelectorAll('button').forEach(function(b) { b.disabled = true; });
        }
        function unlockButtons() {
            pdoc.querySelectorAll('button').forEach(function(b) { b.disabled = false; });
        }

        // Ré-locker les boutons recréés par Streamlit pendant le délai
        var observer = new MutationObserver(function() {
            if (Date.now() < lockUntil) lockButtons();
        });
        observer.observe(
            pdoc.querySelector('[data-testid="stAppViewContainer"]') || pdoc.body,
    ...
    } catch(err) { /* silencieux si cross-origin */ }
})();
</script>
""", height=1)
